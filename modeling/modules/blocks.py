...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.tube_size = config.model.vq_model.enc_tube_size
        ...
        self.positional_embedding = nn.Parameter(
                scale * torch.randn(self.tube_size* (self.grid_size ** 2) + 1, self.width))
    ...
    def forward(self, pixel_values, latent_tokens):
        ...
        latent_tokens = x[:, 1+self.tube_size*(self.grid_size**2):]
...
class TiTokDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.ffn = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Conv2d(2 * self.width, self.output_size, 1, padding=0, bias=True),
        )
    ...
    def forward(self, z_quantized):
        ...
        mask_tokens = self.mask_token.repeat(batchsize, self.tube_size*(self.grid_size**2), 1).to(x.dtype)
        ...
        for i in range(self.num_layers):
            x = self.transformer[i](x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = x[:, 1:1+self.tube_size*(self.grid_size**2)] # remove cls embed
        x = self.ln_post(x)
        # N L D -> N D H W
        # x = x.permute(0, 2, 1).reshape(batchsize, self.width, self.grid_size, self.grid_size)
        x = x.permute(0, 2, 1).reshape(batchsize, self.width, self.tube_size, self.grid_size**2)
        x = self.ffn(x.contiguous())
        x = self.conv_out(x) # [B, D, T, H*W]
        x = x.reshape(batchsize, x.shape[1], self.tube_size, self.grid_size, self.grid_size)
