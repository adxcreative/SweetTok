...
from data import SimpleImageDataset, SimpleVideoDataset, SimpleVideoGenerationDataset
import torch
...
def get_sweet_tokenizer(config):
    tokenizer = SweetTok_Compact(config)
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if config.model.vq_model.model_type == 'SweetTok_base':
        ...
        loss_cls = ReconstructionLoss_Stage2 if config.model.vq_model.finetune_decoder else ReconstructionLoss_Stage1
    elif config.model.vq_model.model_type == 'transformer':
        model_cls = Net2NetTransformer
        loss_cls = MLMLoss
    else:
    ...
    if accelerator.is_main_process:
        if model_type in ['sweettok']:
            ...
            input_size = (1, config.model.vq_model.num_spatial_latent_tokens)
            input_data = [
...
def create_optimizer(config, logger, model, loss_module, need_discrminator=True):
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        # if eval_dataloader is not None and (global_step) % config.experiment.eval_every == 0:
        ...
        with accelerator.accumulate([model, loss_module]):
            # print('5')
            # reconstructed_images, reconstructed_images_, quantized_states, extra_results_dict = model(images)
            # decoded, quantized_states_ = pretrained_tokenizer.decode(reconstructed_images_)
            # import pdb; pdb.set_trace()
            reconstructed_images, extra_results_dict = model(images)
            # reconstructed_images = model.module.pixel_decoder(quantized_states, False)
            if proxy_codes is None:
                ...
                # print('7')
                # print(reconstructed_images.shape, proxy_codes.shape)
                autoencoder_loss, loss_dict = loss_module(
                    proxy_codes,
                    reconstructed_images,
                    extra_results_dict
...
def train_one_epoch_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if "image" in batch:
            ...
            conditions = batch["image"].to(
                accelerator.device, memory_format=torch.contiguous_format, non_blocking=True
            )


            input_tokens = batch['indices'].to(
                accelerator.device, memory_format=torch.contiguous_format, non_blocking=True
            )
            ...
            # import pdb; pdb.set_trace()
            #     for fname, indice in zip(fnames, input_tokens):
            #         out[fname] = indice
        ...
        with accelerator.accumulate([model]):
            logits, targets = model(input_tokens, conditions)
            loss, loss_dict= loss_module(logits, targets)
            # Gather the losses across all processes for logging.
            mlm_logs = {}
            for k, v in loss_dict.items():
    ...
    # with open('./STOKS_indices.pkl', 'wb') as handle:
...
def eval_reconstruction(
    ...
    for batch in eval_loader:
        ...
        # print(original_images.shape, reconstructed_images.shape)
        # if original_images.ndim == 4:
        #     evaluator.update(original_images, reconstructed_images.squeeze(2), model_dict["min_encoding_indices"])
        # else:
        #     evaluator.update(original_images[:, :, 0, :, :], reconstructed_images[:, :, 0, :, :], model_dict["min_encoding_indices"])
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    # print(enc_tokens.shape, reconstructed_images.shape)
    if pretrained_tokenizer is not None:
        reconstructed_images = pretrained_tokenizer.decode(reconstructed_images.argmax(1))
        # reconstructed_images = pretrained_tokenizer.decode(
        #     pretrained_tokenizer.encode(original_images)
        # )
    # print(original_images.shape, reconstructed_images.shape)
...
if __name__ == '__main__':
    pretrained_tokenizer = PretrainedTokenizer_Omni("/data/tanzhentao/pretrain_omni_mlp/imagenet_ucf.ckpt")
    tokens = torch.load('tokens.pth')
    import pdb; pdb.set_trace()
    out = pretrained_tokenizer.decoder(tokens.cpu(),False)
    import pdb; pdb.set_trace()