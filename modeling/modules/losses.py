...
class ReconstructionLoss_Stage1(torch.nn.Module):
    def __init__(
        ...
        self.target_codebook_size = config.model.vq_model.dec_output_size
...
class ReconstructionLoss_Stage2(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        loss_config = config.losses
        self.video_discriminator = NLayerDiscriminator3D(3,64,3,'batch')
    ...
    def _forward_generator(self,
        ...
        if self.reconstruction_loss == "l1":
            reconstruction_loss = F.l1_loss(inputs, reconstructions, reduction="mean") *4
        elif self.reconstruction_loss == "l2":
            reconstruction_loss = F.mse_loss(inputs, reconstructions, reduction="mean")*4
        else:
        ...
        frame_idx = torch.randint(0, T, [B]).to(device = inputs.device)
        ...
        frames = torch.clamp(frames + 0.5, 0.0, 1.0)
        frames_recon = torch.clamp(frames_recon + 0.5, 0.0, 1.0)

        # Compute perceptual loss.
        # perceptual_loss = self.perceptual_loss(inputs, reconstructions).mean()
        perceptual_loss = self.perceptual_loss(frames, frames_recon).mean()

        # Compute discriminator loss.
        generator_loss = torch.zeros((), device=inputs.device)
        gan_feat_loss = torch.zeros((), device=inputs.device)
        ...
        if discriminator_factor > 0.0 and self.discriminator_weight > 0.0:
            ...
            logits_fake, pred_fake = self.video_discriminator(reconstructions)
            generator_loss = -torch.mean(logits_fake)

        # Compute gan feature matching loss
            video_gan_feat_loss = 0
            feat_weights = 4.0 / (3 + 1)
            
            logits_video_real, pred_video_real = self.video_discriminator(inputs, False)
            for i in range(len(pred_fake)-1):
                video_gan_feat_loss += feat_weights * F.l1_loss(pred_fake[i], pred_video_real[i].detach()) * 0.01

            gan_feat_loss = discriminator_factor * 4 * video_gan_feat_loss
        ...
        total_loss = (
            ...
            + d_weight * discriminator_factor * generator_loss + gan_feat_loss
        )
        loss_dict = dict(
            total_loss=total_loss.clone().detach(),
            reconstruction_loss=reconstruction_loss.detach(),
    ...
    def _forward_discriminator(self,
        ...
        # real_images = torch.clamp(rearrange(real_images, 'b c t h w -> (b t) c h w') + 0.5, 0.0, 1.0)
        # reconstructions = torch.clamp(rearrange(reconstructions, 'b c t h w -> (b t) c h w') + 0.5, 0.0, 1.0)
        logits_real,_ = self.video_discriminator(real_images)
        logits_fake,_ = self.video_discriminator(reconstructions.detach())

        discriminator_loss = discriminator_factor * hinge_d_loss(logits_real=logits_real, logits_fake=logits_fake) *0.01
...
class MLMLoss(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        ...
        correct_tokens = ((torch.argmax(inputs, dim=1) == targets)).sum(dim=1)/(targets.shape[0]*targets.shape[1])
