...
def make_viz_from_samples(
    ...
    to_stack = [original_images, reconstructed_images, diff_img] # [B, D, T, H, W]
    ...
    if original_images.ndim == 4:
        ...
        images_for_logging = None
        stacked_image = torch.cat(to_stack, -1)
        images_for_saving = [stacked_image[i].permute(1,0,2,3).numpy().astype('uint8') for i in range(stacked_image.shape[0])]
