...
def parse_codec(txn, video_path):
    codec = pickle.loads(txn.get(video_path.encode('utf-8')))
    return torch.Tensor(codec).int()    
...
class DecordVideoDataset(data.Dataset):
    ...
    def __init__(self, data_folder, data_list=None, fps=None, sequence_length=17, train=True, resolution=64, resizecrop=False,
        ...
        self.train = train
        self.data_folder = data_folder
        # self.data_gt = '/data/xueben/1dtokenizer/pretrained/lmdb_codec_ucf101'
        self.fps = fps
        ...
        self.norm = VideoNorm(mean=normalize_mean, std=normalize_std)
    ...
    def open_lmdb(self):
        self.video_codec_env_gt = lmdb.open(self.data_gt, lock=False, readonly=True, create=False,
                readahead=False, meminit=False,)
        self.video_codec_txn_gt = self.video_codec_env_gt.begin(buffers=True)
    ...
    def __getitem__(self, idx):
        # if not hasattr(self, 'video_codec_txn'):
        #     self.open_lmdb()
        ...
        if self.train:
            if video_path.endswith("webm"):
                ...
                try:
                    frames, _ = load_video_from_path_decord(video_path,frm_sampling_strategy="rand",fps = self.fps if self.fps is not None else -1,num_frm=self.sequence_length,height=read_height,width=read_width,) # T, H, W, 3
                except:
                    print(video_path)
...
class DecordVideoDataset_indice(data.Dataset):
    """ Generic dataset for videos files stored in folders
    Returns BCTHW videos in the range [-0.5, 0.5] """
    exts = ['avi', 'mp4', 'webm']

    def __init__(self, data_folder, data_list=None, tokens=None,fps=None, sequence_length=17, train=True, resolution=64, resizecrop=False,
                    normalize_mean=[0.0, 0.0, 0.0], normalize_std=[1.0, 1.0, 1.0]):
        """
        Args:
            data_folder: path to the folder with videos. The folder
    ...
    def __getitem__(self, idx):
        ...
        return {"image": video, 'class_id': label, "__key__": video_key, "indices": indices}
