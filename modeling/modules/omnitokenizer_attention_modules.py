...
class Transformer(nn.Module):
        ...
        *,
        ...
        for i in range(depth):
            if block[i] == 't':
                self.layers.append(nn.ModuleList([
                    ...
                    Attention(dim=dim, dim_head=dim_head, heads=heads,
                            causal=causal, dropout=attn_dropout, spatial_pos=spatial_pos, norm_context=False),
                    Attention(dim=dim, dim_head=dim_head, dim_context=dim_context, spatial_pos = None, heads=heads, causal=False,
            ...
            # various pooling methods: B, N, C
                ...
                self.layers.append(nn.ModuleList([None,
                    Attention(dim=dim, dim_head=dim_head, heads=heads,
                            causal=causal, dropout=attn_dropout, spatial_pos= None, norm_context=False),
                    Attention(dim=dim, dim_head=dim_head, dim_context=dim_context, heads=heads, causal=False,
                            num_null_kv=0, spatial_pos= None, dropout=attn_dropout),
                    FeedForward(dim=dim, mult=ff_mult, dropout=ff_dropout)        
                ]))
            else:
                raise NotImplementedError
    ...
    def forward(
        ...
        multi_cross = False,
        single_layer = -1,
        external_temporal_layer = None,
        external_temporal_inputs = [],
        external_temporal_layer_nums = [],
        return_norm = True,
    ):
        if return_inter:
            inter_tokens = []
        ...
        for blk, (peg, self_attn, cross_attn, ff) in zip(self.block, self.layers):
            ...
            if not exists(self_attn):
                pass
            elif isinstance(self_attn, Attention):
            ...
            if exists(cross_attn) and exists(context):
                if multi_cross:
                    x = cross_attn(x, context=context[i],
            ...
            if external_temporal_layer is not None and len(external_temporal_layer_nums) > 0 and i in external_temporal_layer_nums:
                # print(ex_idx)
                x = rearrange(x, '(b t) (h w) d -> (b h w) t d', t=video_shape[1], h=video_shape[2], w=video_shape[3])
                x = external_temporal_layer[ex_idx](x, context=external_temporal_inputs[ex_idx], video_shape=video_shape, is_spatial=False,
                    multi_cross=False, single_layer=-1, return_norm=False)
                x = rearrange(x, '(b h w) t d -> (b t) (h w) d', t=video_shape[1], h=video_shape[2], w=video_shape[3])
                ex_idx += 1

            if return_inter:
                inter_tokens.append(x)
            i += 1

        if return_inter:
            inter_tokens.append(self.norm_out(x))
            return inter_tokens
